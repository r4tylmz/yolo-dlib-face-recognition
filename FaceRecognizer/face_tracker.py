...
while True:
    ...
    for i in constants.cams_in_use:
        if grabbed[i]:
            frames[i] = imutils.resize(frames[i], width=750)
    ...
    # register the recognized staffs and show their bouding boxes
    helpers.show_recognized_faces(frames)
    # show on the screen yolo bouinding boxes that are generated by the pre-trained yolo head model
